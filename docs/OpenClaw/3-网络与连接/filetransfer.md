...
假设开发板 IP 为 `192.168.1.100`，用户名为 `openclaw`。
...
```bash
scp ./app_test openclaw@192.168.1.100:~/downloads/
```
...
```bash
scp openclaw@192.168.1.100:/var/log/syslog .
```
...
```bash
scp -r ./src openclaw@192.168.1.100:~/workspace/
```
